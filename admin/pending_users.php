...
if(isset($_SESSION['Username']))
	
 {
    ...
    $sql = $conn->prepare("SELECT * FROM Users WHERE GroupID != 1 AND RegistrationStatus = 0");
...
?>
	...
	{ 
		echo "<td>". "<img src='uploads\user_image\\" . $user['UserImage'] ."'>". "</td>";
									...
									echo "<td>
										<a href='member_edit.php?userid=" . $user['UserID'] . "' 
										class='btn btn-primary'><i class='fa fa-edit'></i> Edit</a>
										<a href='member_delete.php?userid=" . $user['UserID'] . "'
										class='btn btn-danger'><i class='fa fa-close'></i> Delete </a>";
										if ($user['RegistrationStatus'] == 0) {
											echo "<a 
													href='member_activate.php?userid=" . $user['UserID'] . "'
	...
	<a href="member_add.php" class="new-user btn btn-success">
...
<?php
	...
	
	{
			 ...
			 $pending_users_error = "<span class='message-error alert alert-danger'>You Are Not Permited To Access This Page Directly</span>";
			 
			 redirect_home($pending_users_error, 5);
