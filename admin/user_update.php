...
if($_SERVER["REQUEST_METHOD"] == "POST")
 ...
 /* The Allowed Extensions Of Images That Are Going To Be Uploaded */
	 ...
	 if(empty($email))
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Email Field <strong>Can Not Be Empty</strong></div>';
		 }
	 if(empty($fullname))
		 {
...
$sql = $conn->prepare
    ("SELECT * FROM users WHERE Username = '{$username}' && UserID != '{$userid}' ");
...
$count = $sql->rowCount();
	 ...
	 if($count == 1)
	 {
		 $user_update_error = "<div class ='alert alert-danger'>This Username Is Already Existed, Please Select Another One</div>";
		 redirect_users($user_update_error, 5);
	 }
	 else
	 {
...
$sql->execute(array($username, $email, $password, $fullname, $image_name_random, $userid));
			
			$user_update_success = "<span class='alert alert-success'>Record Updated Successfully</span>";
			
			redirect_users($user_update_success);
	...
	
		 {
			 ...
			 $user_update_error = "<span class='message-error alert alert-danger'>You Are Not Permited To Access This Page Directly</span>";
			 
			 redirect_home($user_update_error, 5);
