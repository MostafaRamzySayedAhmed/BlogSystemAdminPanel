...
if($_SERVER["REQUEST_METHOD"] == "POST")
	
 {
	...
	echo "<h1 class='edit text-center'>New User</h1>";
 ...
 /* The Allowed Extensions Of Images That Are Going To Be Uploaded */
  ...
  $form_errors = array();
	 ...
	 if(empty($username))
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Username Field <strong>Can NOT Be Empty</strong></div>';
		 }
	 if(strlen($username) < 5)
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Username Field <strong>Can NOT Be Less Than 5 Characters</strong></div>';
		 }
	 if(strlen($username) > 25)
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Username Field <strong>Can NOT Be More Than 25 Characters</strong></div>';
		 }
	 if(empty($password))
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Password Field <strong>Can NOT Be Empty</strong></div>';
		 }
	 if(empty($email))
		 {
	 ...
	 if(empty($fullname))
		 {
			 $form_errors[] = '<div class="alert alert-danger">The Fullname Field <strong>Can NOT Be Empty</strong></div>';
		 }
	 if(empty($image_name))
		 {
	 ...
	 /*
	 ** Checking If The Username The User Trys To Add Is Already Existed Or Not
    ...
    $count = $sql->rowCount();
	 ...
	 if($count > 0)
	 {
		 $username_message_error = "<div class ='alert alert-danger'>This Username Is Already Existed, Please Select Another One</div>";
		 
		 
		 redirect_user_add($username_message_error, 5);
	 ...
	 else
	 {
			...
			if(empty($form_errors))
			{
                ...
                $sql = $conn->prepare("INSERT INTO 
						...
						users (Username, Password, Email, Fullname, RegistrationStatus, UserDateTime, UserImage)

					VALUES(:name, :pass, :email, :fullname, :status, :date, :image)");
					...
					redirect_users($insert_message_success);	
	...
	
		 {
			 ...
			 $user_insert_error = "<span class='message-error alert alert-danger'>You Are Not Permited To Access This Page Directly</span>";
			 
			 redirect_home($user_insert_error);
